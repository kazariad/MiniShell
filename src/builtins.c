...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list) {
    ...
    if (cmd->word_count == 1) {
        ...
        dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: Invalid input\n");
        return -1;
    }

    if (chdir(target_dir) == -1) {
        dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "cd: %s\n", strerror(errno));
        return -1;
    }

    return vars_set("PWD", target_dir);
}
...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list) {
    if (cmd->word_count == 1) {
        // use existing status code
    } else if (cmd->word_count == 2) {
        char *end = cmd->words[1];
        long code = strtol(cmd->words[1], &end, 10);
        if (!*(cmd->words[1]) || *end || code < 0 || code > 255) {
            dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "exit: Invalid status code [0-255]\n");
            return -1;
        }
        params.status = (int) code;
    } else {
        dprintf(get_pseudo_fd(redir_list, STDERR_FILENO), "exit: Invalid input\n");
        return -1;
    }
